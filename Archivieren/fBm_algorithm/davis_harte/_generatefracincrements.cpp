...
void Increments1::generateIncrements1 (double D ,double tau, double alpha)
{
        ...
        double factor= sqrt(D* std::pow(tau ,alpha))/(sqrt(2)*N);
       ...
       for (int ipart=0; ipart < particles; ++ipart)
       {
            for (int idim=0; idim < dimensions; ++idim)
                ...
                for(int i = 0; i < Nextended; ++i)
                {
                    if( i<=N)
                    {
                        innew[i] = sqrt(out[i].real()*(double)N)*(gsl_ran_gaussian(r,1)+icomplex*gsl_ran_gaussian(r,1));
                    }
                    ...
                    innew[0]=sqrt(2*out[0].real()*(double)N)*gsl_ran_gaussian(r,1);
                    innew[N]=sqrt(2*out[N].real()*(double)N)*gsl_ran_gaussian(r,1);
