...
\section{Smoluchowski Problem}
Lets starts our considerations on chemical reactions with the Smoluchowski problem for kinetics of a bi-molecular chemical reaction in solution. The following derivation shell give the kinetics of an easy model for a bi-molecular reaction and motivate convection-diffusion equation.\newline  
The reaction sceme for a bi-molecular reaction with reactants A and B fusion to a complex AB with a rate $k^{+}_{\mathrm{d}}$ and fission back to A and B with a rate $k^{-}_{\mathrm{d}}$  can be written as:
\begin{align*}
...
\end{align}
Now assuming only a single B molecule being at the position $r=0$. Instantaneous reactions occur for $r\leq \sigma \Rightarrow \rho_t(r \leq \sigma)=0$. For $r \rightarrow \infty$ the distribution is than defined as the concentration of particle A $\rho_t(r \rightarrow \infty)=c_A$.
The solutions for $\rho^{s}(r)$ and $j^{s}_t(r)$ for theses boundary conditions are then:
\begin{align}
...
Smoluchowski's problem can be in fact generalised for reaction to occur with a microscopic rate $\lambda_{+}$ within a radius $\sigma$. Derived by R. Erban and S. J. Chapman in \cite{Erban2009}.  However, as a small remark, the microscopic rate has to be distinguished from the macroscopic rate. As an example microscopic rates are related to the Kramers problem, describing the time dependence of a particle passing a potential barrier like commonly in biology protein binding / unbinding. The problem can be described similar to Smoluchwoski's problem \cref{smoluchwoskiproblem}  with similar boundary conditions for $r > \sigma$ but different for $r \leq \sigma$:
\begin{align}
r\leq \sigma \Rightarrow R=\frac{\partial \rho_t(r \leq \sigma)}{\partial t}=-\lambda_{+} \rho_t (r \leq \sigma)
\end{align}
It can also be interpreted as the production term in the continuity equation. In literature it referred to as the convection-diffusion equation:
\begin{align}
...
\end{align}
Not only in the diffusion limit but also for reactions occurring with a certain microscopic rate the law of mass action can describe a reaction. For a given reaction sceme it is generally possible to formulate a system of ODEs. There solutions give the kinetics and concentrations in equilibrium, but no informations on the spatial distribution are present. For easy problems analytical solutions to spatial distribution can be obtained by solving the convection diffusion \cref{convenctiondiffsuion}. \newline
