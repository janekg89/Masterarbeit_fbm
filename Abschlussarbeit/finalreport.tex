...
Brownian motion is a mathematical model for stochastic processes. It could be applied to finance, biology, physics and many more because of no or only week correlations of the underlieng processes.   
...
\item Brownian motion is a Gaussian process with mean $\langle W_t \rangle_x=x$, $W_o=x$.
Since $ \langle(W_{t_i}-W_{t_{i-1}})(W_{t_j}-W_{t_{j-1}}) \rangle_x=0 $  all its increments $\{W_{t_1},W_{t_2}-W_{t_1},...,W_{t_k}-W_{k-1}\}$ are independent.
...
\begin{mydef}
(variable X umbenen und incement X nennen)
Just like standard Brownian motion, fBm is a Gaussian process $\{X_t\}_{t\geq0}: \Omega \rightarrow \mathbb{R}^d$. Therefore it is fully specified by its mean $E[X_t]=0$ and its (co)variance function $Cov[X_t,X_s]\stackrel{stationary} {=}C(X_{t-s},X_0)=E[\delta X_{t-s}^* X_{0}]\propto t^\alpha$ . 
\end{mydef}
In contrast to the covariance function of white noise:
\begin{align}
 Cov[X_t,X_s]=\sigma^2 \delta(t-s)
\end{align}
check more from \cite{qian2003fractional}
...
Three algorithm have been implemented. 
...
\subsubsection{Lowen}
The algorthm which had been used in the pervious elaborations had down draws in terms of convergence close to 0 and close to the overall simulation time. These problems have been overcome by the algorthm of Steven B. Lowen \cite{Lowen1999}.
