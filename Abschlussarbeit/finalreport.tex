...
classical description of reaction diffusion system  can be 
...
Lets starts our considerations on chemical reactions with the Smoluchowski problem for kinetics of a bi-molecular chemical reaction in solution. The following derivation shell give the kinetics of an easy model for a bi-molecular reaction and motivate under certain conditions the reaction diffusion equation.\newline  
The reaction sceme for a bi-molecular reaction with reactants A and B forming an fusion to a complex AB with a rate $k^{+}_{\mathrm{d}}$ and fission back to A and B with a rate $k^{-}_{\mathrm{d}}$  can be written as:
\begin{align*}
\text{\ch{A + B <=>[ $k^{+}_{\mathrm{d}}$ ][ $k^{-}_{\mathrm{d}}$ ] AB }}
\end{align*}
The assumption of free diffusion of particle A and B with the diffusion constants $D_A$ and $D_B$ , respectively and without any interactions between them is introduced for the most straight forward analytical solution. In fact this problem can be described by an modified version of the diffusion \cref{eq:ficks} motivated in the first chapter.
The modification introduces the joint concentration field for a bi-molecular system in a solution:
\begin{align}
...
\end{align}
Now assuming only a single B molecule being at the position $r=0$. Instantaneous reaction occur for $r\leq \sigma \Rightarrow \rho_t(r \leq \sigma)=0$. For $r \rightarrow \infty$ the distribution is than defined as the concentration of particle A $\rho_t(r \rightarrow \infty)=c_A$.
...
\end{itemize}
Still the solution gives the kinetics for a simple bi-molecular system. It is consistent with collision theory, which states that the collision frequency is proportional to the concentrations of the colliding particles. Pioneer work in reaction kinetics by Guldberg and Waage already in 1864 \cite{Waage1986} resulted in the law of mass action, which states the proportionality of the reaction speed to the concentration with a constant rate. It builds up the basis for a description of reaction kinetics in terms of ordinary differential equations (ODEs) like \cref{difkinetics}.\newline
Smoluchowski's problem can be in fact generalised for reaction to occur with a microscopic rate $\lambda_{+}$ within a radius $\sigma$. Derived by R. Erban and S. J. Chapman in \cite{Erban2009}.  However, as a small remark, the microscopic rate has to be distinguished from the macroscopic rate. As an example microscopic rates are related to the Kramers problem, describing the time dependence of a particle passing a potential barrier like commonly in biology protein binding / unbinding. The problem can be described similar to Smoluchwoski's problem \cref{smoluchwoskiproblem}  with similar boundary conditions for $r > \sigma$ and new for $r \leq \sigma$:
\begin{align}
r\leq \sigma \Rightarrow \frac{\partial \rho_t(r \leq \sigma)}{\partial t}=-\lambda_{+} \rho_t (r \leq \sigma)
\end{align}
It can also be interpreted as the production term in the continuity equation. The calculation to the solution of the second-order ODEs can be found in the Appendix \cref{Erban-Chapmann}. The solution shows a again the possibility of describing the overall change of concentration with a macroscopic reaction rate $K_{+}(\lambda_{+},\sigma)$.
\begin{align}
 \text{hier formal}
\end{align}
